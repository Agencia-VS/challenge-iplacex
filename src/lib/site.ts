...
export function nombreEtapa(numero: number, nombre: string): string {
  return numero === 1 ? "Lanzamiento y período de postulaciones" : nombre;
}
...
export function mapEtapaDB(e: EtapaDB, now: Date): Etapa {
  ...
  return {
    numero: e.numero,
    nombre: nombreEtapa(e.numero, e.nombre),
